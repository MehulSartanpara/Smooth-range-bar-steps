...
var rangeWidth = (document.querySelector(".fir-range").value = 40);
var data = (document.querySelector(".new-change").value = 40);
init(data);
function init(val) {
  initRange(val);
  initAnimate(val);
...
function initRange(val) {
  ...
  changeVals();
  document
    .querySelector(".fir-range")
    .addEventListener("input", changeVals.bind(this));
}
...
function changeCounter() {
  ...
  var newPosition = lerp(
    document.querySelector(".fir-counter").style.getPropertyValue("--position"),
    rangeWidth * range,
    0.1
  );
  var logicCounter = amount - textSuperPosition;
  document.querySelector(".fir-counter").textContent = Math.floor(
    logicCounter > 0 && logicCounter < 1 ? amount : textSuperPosition
  );
  if (textSuperPosition < 100) {
    var newSize = lerp(
      document.querySelector(".fir-line").style.getPropertyValue("--size"),
      range,
      0.1
    );
...
window.onload = function () {
  ...
  if (dosc < 100) {
    document.querySelector(".fir-range").setAttribute("max", "100");
    document.querySelector(".fir-range").setAttribute("step", "1");
  } else if (dosc >= 100 && dosc < 250) {
    document.querySelector(".fir-range").setAttribute("max", "250");
    document.querySelector(".fir-range").setAttribute("step", "25");
  } else if (dosc >= 250) {
    document.querySelector(".fir-range").setAttribute("max", "500");
    document.querySelector(".fir-range").setAttribute("step", "50");
  }
};
...
// ANIMAION
var rangeInput = $('.product_rangebar');
var currentValue = parseInt($('.range_bar_tooltip .fir-counter').text());
rangeInput.on('input', function(){
    setTimeout(function(){
      var newValue = parseInt($('.range_bar_tooltip .fir-counter').text());
